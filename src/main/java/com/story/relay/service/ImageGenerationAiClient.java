...
public class ImageGenerationAiClient {
    ...
    public Mono<ImageGenerationResponseDto> generateImage(ImageGenerationRequestDto request) {
        ...
        // 현재는 Mock 응답 반환
        return Mono.just(generateMockImage(request));
        ...
        return imageGenerationAiWebClient.post()
            ...
            .doOnError(e -> log.error("AI server error during image generation: {}", e.getMessage(), e))
            .onErrorMap(e -> new RuntimeException("Image generation failed: " + e.getMessage()))
            .flatMap(imageBytes -> {
                if (imageBytes == null || imageBytes.length == 0) {
                    return Mono.error(new RuntimeException("No image data received from AI server"));
                }
                ...
                return Mono.just(ImageGenerationResponseDto.builder()
                    .imageUrl(imageUrl)
                    .fileKey(fileKey)
    ...
    public Mono<Boolean> checkHealth() {
        ...
        return Mono.just(true); // Mock: always healthy
